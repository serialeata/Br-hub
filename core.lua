return {
    Init = function()
        ...
        getgenv().Window = WindUI:CreateWindow({ Title = "BR Hub | JailBird Edition", Icon = "shield", Author = "by goth", Folder = "BR_Hub" })
        getgenv().Window:EditOpenButton({ Title = "BR Hub | Jailbird", Icon = "terminal", CornerRadius = UDim.new(0,12), StrokeThickness = 2, Color = ColorSequence.new(Color3.fromHex("FF0F7B"), Color3.fromHex("F89B29")), OnlyMobile = false, Enabled = true, Draggable = true })
        getgenv().ConfigManager = getgenv().Window.ConfigManager
        ...
        game:GetService("UserInputService").InputBegan:Connect(function(input, gpe)
            if not gpe and input.KeyCode == Enum.KeyCode.RightControl then
                ...
                pcall(function() getgenv().Window.Main.Visible = uiVisible end)
            end
        end)
    end
}
